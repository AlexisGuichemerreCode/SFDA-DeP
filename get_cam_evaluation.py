...
def get_cam(exp_path, checkpoint_type, dataset, cudaid, split='train', tmp_outd='tmp_outd', path_cam= None, parsedargs=None):
    ...
    pxap_per_image = []  # list of (image_id, pxap)
        ...
        for image, target, image_id in zip(images, targets, index):
            ...
            if target.item() == 1:
                with torch.set_grad_enabled(cam_computer.req_grad):
                    ...
                    cam_performance = cam_computer.compute_and_evaluate_cams_one_image(self, image, target, image_id, image_size)
                    cam_perf_pxap = cam_computer.evaluator.perf_gist[constants.MTR_PXAP]

                    pxap_per_image.append((image_id, float(pxap_val)))


    pxap_per_image_sorted = sorted(
        pxap_per_image,
        key=lambda x: x[1],
        reverse=True)

    out_file = os.path.join(self.args.outd, "pxap_per_image_sorted.txt")

    with open(out_file, "w") as f:
        f.write("image_id\tpxap\n")
        for image_id, pxap in pxap_per_image_sorted:
            f.write(f"{image_id}\t{pxap:.6f}\n")
