...
def get_visaualization(exp_path, target_method, sf_uda_source_folder, checkpoint_type, dataset, cudaid, image_ids_to_draw, split='test', tmp_outd='tmp_outd', parsedargs=None):
    ...
    with open(join(exp_path, 'config_obj_final.yaml'), 'r') as fy:
        ...
        args_dict['pixel_wise_classification'] = True if 'PixelCAM' in args_dict['method'] else False
    ...
    with open(join(path_cl, 'config_model.yaml'), 'r') as fy:
        ...
        if 'PixelCAM' in args_dict['method']:
    ...
    for batch_idx, (images, targets, _, image_ids, _, _, _, _,_) in tqdm(
        enumerate(loaders[constants.TESTSET]), ncols=constants.NCOLS,
        total=len(loaders[constants.TESTSET])):
        ...
        for image, target, image_id in zip(images, targets, image_ids):
            ...
            #     out = model(new_image.cuda())
            #     end_event.record()
            #     torch.cuda.synchronize()
            #     elapsed_time_ms = start_event.elapsed_time(end_event)
            #     print('time on gpu: {} ms'.format(elapsed_time_ms))
            ...
            with torch.no_grad():
                ...
                cam = cam.detach()
                # overlay_cam = show_cam(t2n(cam), use_rgb=True)
                ...
                gt_mask = get_mask(f'/projets/Aguichemerre/datasets/{dataset}',
                           cam_computer.evaluator.mask_paths[image_id],
                           cam_computer.evaluator.ignore_paths[image_id])

                gt_annotation =  Image.open(os.path.join(f'/projets/Aguichemerre/datasets/{dataset}', cam_computer.evaluator.mask_paths[image_id][0]))
                gt_annotation =  Image.open(os.path.join(f'/projets/Aguichemerre/datasets/{dataset}', cam_computer.evaluator.mask_paths[image_id][0]))
...
def fast_eval():
    ...
    parser.add_argument("--target_dataset", type=str, default=None,
                        help="Name of the dataset.", required=True, choices=[constants.CAMELYON512, constants.GLAS, constants.EBHI])
    ...
    for checkpoint_type_extended in base_checkpoint_types:
            ...
            for i, image_id in enumerate(parsedargs.image_ids_to_draw):
                ...
                if i == 0:
                    if 'GradCAMpp' in target_method:
                        ...
                        methd_name_to_print = target_method
        ...
        plt.savefig(os.path.join(out_dir, 'PixelCAM_EBHI.png'))
