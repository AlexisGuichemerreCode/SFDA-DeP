...
def get_visaualization(exp_path, target_method, sf_uda_source_folder, checkpoint_type, dataset, cudaid, image_ids_to_draw, split='test', tmp_outd='tmp_outd', parsedargs=None):
    ...
    if "tscam" in encoder_name:
        ...
        header_pixel = torch.load(join(path_cl, 'pixel_wise_classification_head.pt'),
                            map_location=get_cpu_device())
        model.pixel_wise_classification_head.load_state_dict(header_pixel, strict=True)
...
def fast_eval():
    ...
    base_checkpoint_types = [constants.BEST_LOC, constants.BEST_CL]
    ...
    for checkpoint_type_extended in base_checkpoint_types:
        ...
        for ind_method, target_method in enumerate(target_methods):
            ...
            for i, image_id in enumerate(parsedargs.image_ids_to_draw):
                ...
                if i == 0:
                    ...
                    axs[i, ind_method].set_title(f'{method_name}\n{target_method}', fontsize=18)
                axs[i, ind_method].axis('off')
        ...
        out_dir = os.path.join(out_dir, f'{method_name_lst[0]}_{checkpoint_type}')
        os.makedirs(out_dir, exist_ok=True)
        
        out_dir = os.path.join(out_dir, f'target_ds_{parsedargs.target_dataset}')
        os.makedirs(out_dir, exist_ok=True)

        out_dir = os.path.join(out_dir, f'with_source_best_cl_{parsedargs.draw_vis_with_best_source_classifier}')
        os.makedirs(out_dir, exist_ok=True)
