...
def new_shift(images, path_staining):
    ...
    IMAGE_MEAN_VALUE = torch.tensor([0.485, 0.456, 0.406], device=device).view(3, 1, 1)
    IMAGE_STD_VALUE = torch.tensor([0.229, 0.224, 0.225], device=device).view(3, 1, 1)


    image_denorm = images * IMAGE_STD_VALUE + IMAGE_MEAN_VALUE
    image_denorm = (image_denorm * 255).clamp(0, 255).byte()


    image_cv2 = image_denorm.permute(0, 2, 3, 1).cpu().numpy()  # (B, H, W, C)

    path_staining = Path(path_staining)
    pt_files = list(path_staining.glob("*.pt"))

    if not pt_files:
        raise FileNotFoundError(f"Aucun fichier .pt trouvé dans {path_staining}")

    staining_data = torch.load(pt_files[0], map_location=device) 
    mean_he = torch.tensor(staining_data["mean_he"]).detach().cpu()
    mean_maxC = torch.tensor(staining_data["mean_maxC"]).detach().cpu()


    normalizer_shift = torchstain.normalizers.MacenkoNormalizer(backend='torch')
    normalizer_shift.HERef = mean_he
    normalizer_shift.maxCRef = mean_maxC

    normalized_images = [
        normalizer_shift.normalize(I=transforms.ToTensor()(img), stains=True)[0].permute(2, 0, 1).to(device).float()
        for img in image_cv2
    ]


    normalized_images = torch.stack(normalized_images).to(device)

    # Re-normalisation
    normalized_images = normalized_images / 255.0  # Convertir en [0,1]
    normalized_images = (normalized_images - IMAGE_MEAN_VALUE) / IMAGE_STD_VALUE
...
def get_pixel_shift(exp_path, checkpoint_type, dataset, cudaid, split='train', tmp_outd='tmp_outd', path_staining = None, pixel_wise_classification = False, parsedargs=None):
    # config_model.yaml
    ...
    nb_stain = 20
    results = {i+1: {'mean': [], 'min': [], 'max': [], 'cosine_sim': []} for i in range(nb_stain)}
    ...
    for batch_idx, (images, targets, p_glabel, index, raw_imgs, std_cams, _, views) in tqdm(
        ...
        with torch.no_grad():
            ...
            pixel_features_orig = model.encoder_last_features

        for i in range(nb_stain):
            num_stain = i
            path_staining = parsedargs.path_staining + "/cluster_" + str(i)
            #Images with new shift
            images_shifted = new_shift(images, path_staining=path_staining)

            with torch.no_grad():
                # Extract pixel features from the modified images
                out_shifted = model(images_shifted.cuda())
                pixel_features_shifted = model.encoder_last_features  # (B, C, H, W)

            # compute difference
            #feature_diff = torch.abs(pixel_features_orig - pixel_features_shifted)
            pixel_features_orig = F.normalize(pixel_features_orig, p=2, dim=1)  # Shape: (B, 2048, H, W)
            pixel_features_shifted = F.normalize(pixel_features_shifted, p=2, dim=1)  # Shape: (B, 2048, H, W)

            feature_diff = torch.norm(pixel_features_orig - pixel_features_shifted, p=2, dim=1)

            cos_sim = torch.nn.functional.cosine_similarity(pixel_features_orig, pixel_features_shifted, dim=1)

            mean_cos_sim = cos_sim.mean(dim=(1, 2)).tolist()  # Shape: [32]


            # Compute mean, min and max
            #mean_diff = torch.mean(feature_diff, dim=[1, 2, 3]) 

            #mean_diff_per_pixel  = torch.mean(feature_diff, dim=1)
            mean_feature_diff = feature_diff.mean(dim=(1, 2)).tolist()

            min_diff = torch.amin(feature_diff, dim=[1, 2]).tolist()
            max_diff = torch.amax(feature_diff, dim=[1, 2]).tolist()
             
            #mean_diff_rounded = [round(val, 3) for val in mean_diff.cpu().tolist()]
            #min_diff_rounded = [round(val, 3) for val in min_diff.cpu().tolist()]
            #max_diff_rounded = [round(val, 3) for val in max_diff.cpu().tolist()]

            # Store values
            stain_pos = num_stain + 1
            results[stain_pos]['mean'].extend(mean_feature_diff)  
            results[stain_pos]['min'].extend(min_diff)
            results[stain_pos]['max'].extend(max_diff)
            results[stain_pos]['cosine_sim'].extend(mean_cos_sim) 

    # Average of max min and mean values for each stain
    final_results = {
        num_stain: {
            'mean': sum(values['mean']) / len(values['mean']) if values['mean'] else 0,
            'min': sum(values['min']) / len(values['min']) if values['min'] else 0,
            'max': sum(values['max']) / len(values['max']) if values['max'] else 0,
            'cosine_sim': sum(values['cosine_sim']) / len(values['cosine_sim']) if values['cosine_sim'] else 0
        }
        for num_stain, values in results.items()
    }


    for num_stain, stats in final_results.items():
        print(f"Stain {num_stain}: Mean Diff = {stats['mean']:.9f}, Min Diff = {stats['min']:.9f}, Max Diff = {stats['max']:.9f}, Cos sim= {stats['cosine_sim']:.9f}") 


    stains = list(final_results.keys())
    mean_diffs = [stats['mean'] for stats in final_results.values()]
    min_diffs = [stats['min'] for stats in final_results.values()]
    max_diffs = [stats['max'] for stats in final_results.values()]
    cosine_sim = [stats['cosine_sim'] for stats in final_results.values()]


    plt.figure(figsize=(10, 6))

    plt.plot(stains, mean_diffs, marker='o', linestyle='-', label="Mean Diff", color='blue')
    plt.plot(stains, min_diffs, marker='s', linestyle='--', label="Min Diff", color='green')
    plt.plot(stains, max_diffs, marker='^', linestyle='-.', label="Max Diff", color='red')
    plt.plot(stains, cosine_sim, marker='D', linestyle=':', label="Cosine Similarity", color='purple')

    plt.xlabel("Stain Index")
    plt.ylabel("Feature Distance")
    plt.title("Feature Distance Variation Across Stains")
    plt.legend()
    plt.grid(True)
    plt.xticks(stains, stains)


    plt.savefig("stain_differences_plot_layercam_modif.png", dpi=300, bbox_inches='tight')
...
def fast_eval():
    ...
    parser.add_argument('--domain_exp_path', type=json.loads, default={})
    ...
    for checkpoint_type_extended in base_checkpoint_types:
        ...
        for ind_method, target_method in enumerate(target_methods):
            ...
            if target_method == 'SOURCE':
                ...
                exp_path = parsedargs.domain_exp_path[target_method]


            #Get features at the pixel level

            overlay_images, input_images, method_name, gt_masks = get_pixel_shift(exp_path=exp_path,checkpoint_type=checkpoint_type, dataset=parsedargs.source_dataset, cudaid=parsedargs.cudaid, split=split, tmp_outd='tmp_outd', path_staining = parsedargs.path_staining, pixel_wise_classification=parsedargs.pixel_wise_classification, parsedargs=parsedargs)
