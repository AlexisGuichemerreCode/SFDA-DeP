...
def get_pixel_shift(exp_path, checkpoint_type, dataset, cudaid, split='train', tmp_outd='tmp_outd', path_staining = None, pixel_wise_classification = False, parsedargs=None):
    ...
    nb_stain = 20
    H, W = 28, 28  # taille de la feature map
    pixel_norms = {i+1: [] for i in range(nb_stain)}  # pour chaque stain, une liste de vecteurs de taille 784

    for batch_idx, (images, _, _, _, _, _, _, _) in tqdm(enumerate(loaders[split]), ncols=constants.NCOLS, total=len(loaders[split])):
        images = images.to(device)

        with torch.no_grad():
            _ = model(images.cuda())
            pixel_features_orig = model.encoder_last_features  # (B, C, H, W)

        for i in range(nb_stain):
            num_stain = i + 1
            path_staining = parsedargs.path_staining + f"/cluster_{i}"
            images_shifted = new_shift(images, path_staining=path_staining)

            with torch.no_grad():
                _ = model(images_shifted.cuda())
                pixel_features_shifted = model.encoder_last_features  # (B, C, H, W)

            # Calcule de la norme par pixel : (B, H, W)
            pixel_norms_batch = torch.norm(pixel_features_shifted, p=2, dim=1)  # norme L2 sur la dimension des features

            # On reshape chaque carte en (B, H*W) = (B, 784) puis on empile
            pixel_norms_flat = pixel_norms_batch.view(pixel_norms_batch.size(0), -1).cpu().numpy()  # (B, 784)

            # On ajoute chaque image individuellement à la liste du stain
            pixel_norms[num_stain].extend(pixel_norms_flat)

    # ➤ Moyenne par position (784) pour chaque stain
    mean_pixel_norms_per_stain = {
        stain: np.mean(np.stack(pixel_values), axis=0)  # (784,)
        for stain, pixel_values in pixel_norms.items()
    }

    H, W = 28, 28
    nb_pixels = H * W
    bottom = np.zeros(nb_pixels)  # commence à 0 pour chaque pixel
    plt.figure(figsize=(24, 6))

    for stain in range(1, nb_stain + 1):
        heights = mean_pixel_norms_per_stain[stain]  # (784,)
        plt.bar(np.arange(nb_pixels), heights, bottom=bottom, label=f'Stain {stain}', width=1.0)
        bottom += heights  # on empile les barres

    plt.xlabel('Pixel index (flattened 28x28)', fontsize=14)
    plt.ylabel('Stacked norm value', fontsize=14)
    plt.title('Stacked Mean Norms per Pixel for Each Stain', fontsize=16)
    plt.legend(ncol=4, bbox_to_anchor=(1.05, 1.0), loc='upper left')
    plt.tight_layout()

    # ➤ Save
    plt.savefig("stacked_pixel_norms_histogram.png", dpi=300)
    plt.show()
