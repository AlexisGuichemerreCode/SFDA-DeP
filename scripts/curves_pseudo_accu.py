...
# def plot_topk_curve(all_accuracies, save_path="topk_accuracy_curve.png"):

#     all_accuracies = np.array(all_accuracies)  
#     mean_accuracies = np.mean(all_accuracies, axis=0) 

#     plt.figure(figsize=(8, 6))
#     plt.plot(np.linspace(1, 100, len(mean_accuracies)), mean_accuracies, marker='o', linestyle='-')
#     plt.xlabel("Top-k pixels (%)")
#     plt.ylabel("Accuracy")
#     plt.grid(True)
...
def plot_topk_curve(all_accuracies, all_accuracies_inverted=None, save_path="topk_accuracy_curve.png"):

    plt.figure(figsize=(8, 6))
    x_values = np.linspace(1, 100, 100)

    if all_accuracies:
        all_accuracies = np.array(all_accuracies)  # Convertir en array (N_images, 100)
        mean_accuracies = np.mean(all_accuracies, axis=0)  # Moyenne sur toutes les images
        #plt.plot(np.linspace(1, 100, len(mean_accuracies)), mean_accuracies, marker='o', linestyle='-', color='blue', label="Foreground")
        # all_accuracies = np.array(all_accuracies)  
        # mean_accuracies = np.mean(all_accuracies, axis=0) 
        std_accuracies = np.std(all_accuracies, axis=0)  
        ci = 0.1 * std_accuracies / mean_accuracies  

        plt.plot(x_values, mean_accuracies, marker='o', linestyle='-', color='blue', label="Foreground")
        plt.fill_between(x_values, (mean_accuracies - ci), (mean_accuracies + ci), color='blue', alpha=0.3)  

    if all_accuracies_inverted:
        all_accuracies_inverted = np.array(all_accuracies_inverted)  # Convertir en array (N_images, 100)
        mean_accuracies_inverted = np.mean(all_accuracies_inverted, axis=0)  # Moyenne sur toutes les images
        #plt.plot(np.linspace(1, 100, len(mean_accuracies_inverted)), mean_accuracies_inverted, marker='s', linestyle='--', color='red', label="Background")
        std_accuracies_inverted = np.std(all_accuracies_inverted, axis=0)  
        ci_inverted = 0.1 * std_accuracies_inverted / mean_accuracies_inverted  

        plt.plot(x_values, mean_accuracies_inverted, marker='s', linestyle='--', color='red', label="Background")
        plt.fill_between(x_values, (mean_accuracies_inverted - ci_inverted), (mean_accuracies_inverted + ci_inverted), color='red', alpha=0.3) 

    plt.xlabel("Top-k pixels (%)")
    plt.ylabel("True positive (blue) True negative (red) rate")
    plt.grid(True)
    plt.legend() 
...
def compute_topk_pixel_accuracy(cam, resized_mask_array, value=255, num_topk=100):

    ground_truth = (resized_mask_array == value).astype(np.int32)  # (28, 28)
    ...
    topk_accuracies = []
    for k in range(1, num_topk + 1): 
        num_pixels = int(k * len(sorted_indices) / num_topk) 
        selected_indices = sorted_indices[:num_pixels] 
        correct = ground_truth_flat[selected_indices].sum()  
        accuracy = correct / num_pixels
...
def _compute_accuracy(args, model, loader, list_stain, maxC_list):
    ...
    for i, (images, targets, _, _, _, _, _, _) in enumerate(loader):
        ...
        for image in images:
            ...
            # Apply normalization
            normalized_image, H, E = normalizer_shift.normalize(I=T(image_np), stains=True)
...
def get_visaualization(exp_path, target_method, sf_uda_source_folder, checkpoint_type, dataset, cudaid, image_ids_to_draw, split='test', tmp_outd='tmp_outd', parsedargs=None):
    ...
    cam_computer = CAMComputer(
            args=deepcopy(args),
            model=model,
            loader=loaders[constants.TRAINSET],
            metadata_root=os.path.join(metadata_root, constants.TRAINSET),
    ...
    maxC_list = []
    all_accuracies = []
    all_accuracies_inverted = []

    for batch_idx, (images, targets, _, image_ids, _, _, _, _) in tqdm(
        enumerate(loaders[constants.TRAINSET]), ncols=constants.NCOLS,
        total=len(loaders[constants.TRAINSET])):
        ...
        for image, target, image_id in zip(images, targets, image_ids):
            if target.item() == 1 :
                ...
                if target.item() == 1:
                    ...
                    topk_accuracies = compute_topk_pixel_accuracy(cam, resized_mask_array, value = 255)
                    all_accuracies.append(topk_accuracies)
                    plot_topk_curve(all_accuracies, save_path="test.png")
                else:
                    m,n = cam.shape
                    ...
                    cam_inverted = 1 - cam 
                    topk_accuracies_inverted = compute_topk_pixel_accuracy(cam_inverted, resized_mask_array, value = 0)
                    all_accuracies_inverted.append(topk_accuracies_inverted)
                


    plot_topk_curve(all_accuracies, save_path="accuracy_curve_foreground_camelyon_gradcampp.png")
...
def fast_eval():
    ...
    # Charger les images
    img1 = mpimg.imread("accuracy_curve_glas_gradcampp.png")
    img2 = mpimg.imread("accuracy_curve_foreground_camelyon_gradcampp.png")

    # Créer une figure avec 2 sous-figures côte à côte
    fig, axes = plt.subplots(1, 2, figsize=(12, 6))  # 1 ligne, 2 colonnes


    axes[0].imshow(img1)
    axes[0].set_title("Accuracy Curve - GLAS - GradCAM++")
    axes[0].axis("off")  


    axes[1].imshow(img2)
    axes[1].set_title("Accuracy Curve - CAMELYON - GradCAM++")
    axes[1].axis("off")  
    ...
    plt.savefig("combined_accuracy_curves.png", dpi=300, bbox_inches='tight')
    ...
    for checkpoint_type_extended in base_checkpoint_types:
        ...
        for ind_method, target_method in enumerate(target_methods):
            ...
            for i, image_id in enumerate(parsedargs.image_ids_to_draw):
                ...
                if i == 0:
                    if 'GradCAMpp' in target_method:
                        methd_name_to_print = 'GradCAM++'
                    elif 'EnergyCAM DL' in target_method:
                        methd_name_to_print = 'PixelCAM\nDeepmil'
                    elif 'EnergyCAM GC' in target_method:
                        methd_name_to_print = 'PixelCAM\nGradCAM++'
                    elif 'EnergyCAM LC' in target_method:
                        methd_name_to_print = 'PixelCAM\nLayerCAM'
                    elif 'EnergyCAM SAT' in target_method:
                        methd_name_to_print = 'PixelCAM\nSAT'
                    else:
                        methd_name_to_print = method_name
                    # methd_name_to_print = method_name.upper().replace('GRADCAMPP', 'GRADCAM++')
                    # methd_name_to_print = method_name.replace('EnergyCAM DL', 'PixelCAM/nDeepmil')
                    # methd_name_to_print = method_name.replace('EnergyCAM GC', 'PixelCAM/nGradCAM++')
                    # methd_name_to_print = method_name.replace('EnergyCAM LC', 'PixelCAM/nLayerCAM')
                    axs[i, ind_method].set_title(f'{methd_name_to_print}', fontsize=18)
                axs[i, ind_method].axis('off')
        ...
        plt.savefig(os.path.join(out_dir, 'PixelCAM_CAMELYON_target_cancer.png'))
