...
def compute_distances_by_predicted_class_and_correctness(energy_data, model):
    """
    ...
    distances = np.array([
        norm(f - anchors[1 - p])  # distance à l'ancre opposée à la prédiction p
        for f, p in zip(features, preds)
    ])

    results = {
        'class_0_correct': distances[(preds == 0) & (preds == labels)],
...
def get_features(exp_path, sf_uda_source_folder,image_ids_to_draw,image_ids_to_draw_target, checkpoint_type, source_dataset,target_dataset, cudaid, split, tmp_outd='tmp_outd', parsedargs=None):
    ...
    plt.hist(results['class_0_correct'], bins=50, alpha=0.7, label='Correctly Predicted')
    plt.hist(results['class_0_incorrect'], bins=50, alpha=0.7, label='Incorrectly Predicted')
    plt.title("Distances to Opposite Anchor (Predicted Class: 0 - Normal)")
    plt.xlabel("Distance to Opposite Anchor")
    plt.ylabel("Number of Images")
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "distances_class_0_cam_test.png"), dpi=300)

    # Classe prédite = 1
    plt.figure(figsize=(6, 4))
    plt.hist(results['class_1_correct'], bins=50, alpha=0.7, label='Correctly Predicted')
    plt.hist(results['class_1_incorrect'], bins=50, alpha=0.7, label='Incorrectly Predicted')
    plt.title("Distances to Opposite Anchor (Predicted Class: 1 - Cancer)")
    plt.xlabel("Distance to Opposite Anchor")
    plt.ylabel("Number of Images")
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "distances_class_1_cam_test.png"), dpi=300)
