...
def measure_accuracy_diff_model(exp_path_source,exp_path_target, checkpoint_type, source_dataset,target_dataset, cudaid, split, tmp_outd='tmp_outd', parsedargs=None, args = None):
    ...
    # Avant normalisation
    source_image_anchor_weights = source_model.classification_head.fc.weight[1:].detach().cpu().numpy()
    target_image_anchor_weights = target_model.classification_head.fc.weight[1:].detach().cpu().numpy()

    # Normaliser
    Wb = source_image_anchor_weights / (np.linalg.norm(source_image_anchor_weights, axis=1, keepdims=True) + 1e-12)
    Wa = target_image_anchor_weights / (np.linalg.norm(target_image_anchor_weights, axis=1, keepdims=True) + 1e-12)
    cos_sim = np.sum(Wb * Wa, axis=1)
    ...
    if class_names is None:
        class_names = [f"Classe {i}" for i in range(len(angles))]

    # --- Plot ---
    plt.figure(figsize=(6,5))
    plt.bar(class_names, angles, color=['blue','red'])
    for i, a in enumerate(angles):
        plt.text(i, a+1, f"{a:.2f}°", ha='center', va='bottom')

    plt.ylabel("Angle (degrés)")
    plt.title("Variation de l'angle des poids avant/après unlearning")
    plt.ylim(0, max(angles)+10)
    plt.grid(axis='y', linestyle='--', alpha=0.6)
    plt.savefig("angle_variation.png", dpi=300, bbox_inches="tight")
    plt.show()
