...
def _compute_accuracy_stats(args, model,split, loaders):
    model.eval()
    ...
    # global accuracy
    acc_global = 100.0 * num_correct / max(1, num_images)

    # accuracy par classe
    acc_per_class = {
        c: 100.0 * correct_per_class[c] / max(1, total_per_class[c])
        for c in total_per_class
    }

    # ratio de prédictions
    pred_ratio = {
        c: pred_count[c] / max(1, num_images)
        for c in pred_count
    }

    return acc_global, acc_per_class, pred_ratio
...
def reduce_valcl_half_gt(src_fold: Path, dst_fold: Path, seed=0):
    random.seed(seed)
    ...
    data = {f: open(src / f).readlines() for f in files}
    labels = [int(l.strip().split(",")[-1]) for l in data["class_labels.txt"]]
    ...
    selected = []
    for c in [0, 1]:
        selected += random.sample(idx_by_class[c], len(idx_by_class[c]) // 2)

    selected = sorted(selected)

    for f in files:
        with open(dst / f, "w") as fw:
            fw.writelines([data[f][i] for i in selected])



def reduce_train_half_random(src_fold: Path, dst_fold: Path, seed=0):
    random.seed(seed)
    ...
    data = {f: open(src / f).readlines() for f in files}
    n = len(data["image_ids.txt"])

    selected = sorted(random.sample(range(n), n // 2))
...
def print_gt_distribution(fold_root: Path, split="train"):
    label_file = fold_root / split / "class_labels.txt"
...
def get_features(exp_path, sf_uda_source_folder,image_ids_to_draw,image_ids_to_draw_target, checkpoint_type, dataset, cudaid, split, tmp_outd='tmp_outd', parsedargs=None, target_method=None):
    ...
    #acc_global, acc_per_class, pred_ratio = _compute_accuracy_stats(args, model,split, loaders)


    # ============================================================
    # 2) Créer un nouveau fold SANS BIAIS
    # ============================================================

    ROOT = Path("folds/wsol-done-right-splits") / dataset
    SRC_FOLD = ROOT / f"fold-{parsedargs.fold_dataset}"

    # nouveau fold id (choisis ce que tu veux)
    fold_id = parsedargs.fold_dataset + 1000
    DST_FOLD = ROOT / f"fold-{fold_id}"

    print(f"[CREATE FOLD {DST_FOLD.name}]")

    # 1) Copier test et valpx tels quels
    for split_name in ["test", "valpx"]:
        shutil.copytree(
            SRC_FOLD / split_name,
            DST_FOLD / split_name,
            dirs_exist_ok=True
    ...
    # 2) Réduire valcl (GT-aware)
    reduce_valcl_half_gt(
        src_fold=SRC_FOLD,
        dst_fold=DST_FOLD,
        seed=0
    )

    # 3) Réduire train (SFDA-safe, aléatoire)
    reduce_train_half_random(
        src_fold=SRC_FOLD,
        dst_fold=DST_FOLD,
        seed=0
    )

    FOLD = Path("folds/wsol-done-right-splits/CAMELYON17_512/fold-1003")
    print_gt_distribution(FOLD, split="train")
    print_gt_distribution(FOLD, split="valcl")
    print_gt_distribution(FOLD, split="test")
