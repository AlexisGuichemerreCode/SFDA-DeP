...
def get_cam(exp_path, checkpoint_type, dataset, cudaid, split='train', tmp_outd='tmp_outd', path_cam= None, parsedargs=None):
    ...
    with open(join(path_cl, 'config_model.yaml'), 'r') as fy:
        ...
        # args_dict = yaml.safe_load(fy)
        # args_dict['model']['freeze_encoder'] = False
        args_dict['pixel_wise_classification'] = parsedargs.pixel_wise_classification
    ...
    #         #pre_tr_state = torch.load(path_cl, map_location=get_cpu_device(), weights_only=False)['full_model']
    #         pre_tr_state = torch.load(join(path_cl, 'model.pt'), map_location=get_cpu_device(), weights_only=False)
    #         model.load_state_dict(pre_tr_state['full_model'], strict=args.model['strict'])
    ...
    metadata_root = join(constants.RELATIVE_META_ROOT, dataset, f"fold-{parsedargs.fold_compute_cam}")
    ...
    loaders = get_data_loader(
            ...
            eval_batch_size = 32,
            #args=args#args.eval_batch_size,
        )   
...
def fast_eval():
    ...
    parser.add_argument("--fold_compute_cam", type=int, default=None, help="Path to store CAMs.")
