...
from dlib.gmm.utils import calculate_matmul, calculate_matmul_n_times
...
class GaussianMixture(torch.nn.Module):
    ...
    def _init_params(self):
        ...
        # (1, k, 1)
        if hasattr(self, 'pi_init') and self.pi_init is not None:
            assert self.pi_init.size() == (1, self.n_components, 1), "Input var_init does not have required tensor dimensions (1, %i, 1)" % (self.n_components)
            self.pi = torch.nn.Parameter(self.pi_init, requires_grad=False)
        else:
            self.pi = torch.nn.Parameter(torch.full((1, self.n_components, 1), 1. / self.n_components), requires_grad=False)
    ...
    def fit(self, x, delta=1e-3, n_iter=100, warm_start=False):
        ...
        while (i <= n_iter) and (j >= delta):
            ...
            i += 1
            j = abs(self.log_likelihood - log_likelihood_old)
