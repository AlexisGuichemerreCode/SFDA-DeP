...
def get_features(exp_path, sf_uda_source_folder,image_ids_to_draw,image_ids_to_draw_target, checkpoint_type, source_dataset, target_dataset, cudaid, split, tmp_outd='tmp_outd', parsedargs=None, target_method=None, n_components=2):
    ...
    source_array = np.array(source_img_features)
    target_array = np.array(target_img_features)

    with torch.no_grad():
        if args.model['support_background']:
            anchors = model.classification_head.fc.weight[1:]
    ...
    plt.scatter(
        features_tsne_source[np.array(labels) == 0, 0],
        features_tsne_source[np.array(labels) == 0, 1],
        label='Source',
        alpha=1.0,
        s=10,
        c='cornflowerblue',
    ...
    # Anchors
    plt.scatter(features_tsne_source[np.array(labels) == 1, 0], features_tsne_source[np.array(labels) == 1, 1], 
            label='Anchor - Class 0', c='black', s=100, marker='P')

    plt.scatter(features_tsne_source[np.array(labels) == 2, 0], features_tsne_source[np.array(labels) == 2, 1],
            label='Anchor - Class 1', c='red', s=100, marker='P')

    plt.legend()
    plt.title("t-SNE 2D des features du domaine source par classe")
    plt.xlabel("TSNE-1")
    plt.ylabel("TSNE-2")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("tsne_source_only.png", dpi=300, bbox_inches="tight")
    plt.show()


    # t-SNE
    features_tsne = TSNE(n_components=2, perplexity=5, random_state=42, min_grad_norm=1e-08, max_iter=2000).fit_transform(features_all)
    features_tsne = np.array(features_tsne)
    labels = np.array(labels)

    source_indices = np.where(labels == 0)[0]
    target_indices = np.where(labels == 1)[0]

    source_tsne = features_tsne[source_indices]
    target_tsne = features_tsne[target_indices]
    ...
    plt.scatter(
        ...
        #source_tsne[np.array(y_source) == 0, 2],
        label='Source - Normal',
        alpha=1.0,
        s=10,
        c='cornflowerblue',
    ...
    plt.scatter(
        ...
        #source_tsne[np.array(y_source) == 1, 2],
        label='Source - Cancer',
        alpha=1.0,
        s=10,
        c='black',
    ...
    plt.scatter(
        ...
        #target_tsne[np.array(y_target) == 0, 2],
        label='Target - Normal',
        alpha=1.0,
        s=10,
        c='lightsalmon',
    ...
    plt.scatter(
        ...
        #target_tsne[np.array(y_target) == 1, 2],
        label='Target - Cancer',
        alpha=1.0,
        s=10,
        c='firebrick',
    ...
    #ax.scatter(features_tsne[labels == 4, 0], features_tsne[labels == 4, 1], features_tsne[labels == 4, 2], 
    #        label='Anchor - Class 1', c='blue', s=100, marker='P')
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("tsne_features_colored_by_class_2_glas_source.png", dpi=300, bbox_inches="tight")
    plt.show()
