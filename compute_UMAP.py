...
def compute_energy_distributions_img(model, loader, cam_computer, dataset_name, energy_fn, split, device, args=None, metadata_root=None, cam_performance=False):
...
def sample_balanced(features, labels, n_per_class=1000, seed=42):
    np.random.seed(seed)
    ...
    for c in unique_classes:
        ...
        # si moins que n_per_class, on prend tout
        n_select = min(n_per_class, len(idx_c))
        idx_sample = np.random.choice(idx_c, size=n_select, replace=False)

        sampled_features.append(features[idx_sample])
        sampled_labels.append(labels[idx_sample])

    # concaténer les sous-échantillons
...
def separability_J(features, labels):
    """
    features: array (N, D)
    labels: array (N,)
    """
    ...
    for c in classes:
        ...
        Sb += n_c * (diff @ diff.T)
        # Intra-classe
        diff_intra = Xc - mean_c
        Sw += diff_intra.T @ diff_intra

    # Rapport des traces
    J = np.trace(Sb) / (np.trace(Sw) + 1e-12)
    return J
...
def get_features(exp_path, sf_uda_source_folder,image_ids_to_draw,image_ids_to_draw_target, checkpoint_type, source_dataset,target_dataset, cudaid, split, tmp_outd='tmp_outd', parsedargs=None):
    ...
    if target_dataset == constants.CUB:
        ...
        target_energy = compute_energy_distributions_img(model, target_loaders, target_cam_computer, target_dataset, energy_fn, split, device, args=args, metadata_root=target_metadata_root, cam_performance = False)
