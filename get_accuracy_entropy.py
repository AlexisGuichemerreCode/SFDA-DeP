...
def collect_entropy_predictions(args, model, loader):
    model.eval()
    ...
    for i, (images, targets, _, _, _, _, _, _, _) in enumerate(loader):
        ...
        with torch.no_grad():
            logits = cl_forward(args, model, images)   # (B, C)
...
def accuracy_vs_entropy_percentile(data, num_points=100):
    entropy = data["entropy"]
    pred = data["pred"]
    target = data["target"]

    # tri par entropie croissante
    idx = torch.argsort(entropy)
    pred = pred[idx]
    target = target[idx]

    n = len(entropy)
    percents = np.linspace(1, 100, num_points)
    acc_curve = []

    for p in percents:
        k = max(1, int(p / 100.0 * n))
        acc = (pred[:k] == target[:k]).float().mean().item() * 100
        acc_curve.append(acc)

    return percents, acc_curve


def accuracy_vs_entropy_per_class(data, class_id, num_points=100):
    mask = data["target"] == class_id

    entropy = data["entropy"][mask]
    pred = data["pred"][mask]
...
def accuracy_vs_entropy_per_predicted_class(data, class_id, num_points=100):
    pred_all = data["pred"]
    target_all = data["target"]
    entropy_all = data["entropy"]

    # 🔹 filtrage par classe prédite
    mask = pred_all == class_id

    if mask.sum() == 0:
        return None, None

    entropy = entropy_all[mask]
    pred = pred_all[mask]
...
def save_and_show_accuracy_plot(
    x,
    acc_global,
    acc_c0=None,
    acc_c1=None,
    save_path="accuracy_vs_entropy.png",
    title=None
):
    plt.figure(figsize=(5, 4))

    plt.plot(x, acc_global, label="Global", linewidth=2)

    if acc_c0 is not None:
        plt.plot(x, acc_c0, "--", label="Class 0")
    if acc_c1 is not None:
        plt.plot(x, acc_c1, "--", label="Class 1")

    plt.xlabel("Top N% lowest-entropy samples")
    plt.ylabel("Accuracy (%)")
    ...
    # 🔹 cleanup (important en loop)
    plt.close()



def build_entropy_plot_path(
    dataset,
    fold,
    split,
    method_name,
...
def get_features(exp_path, sf_uda_source_folder,image_ids_to_draw,image_ids_to_draw_target, checkpoint_type, dataset, cudaid, split, tmp_outd='tmp_outd', parsedargs=None, target_method=None):
    ...
    data = collect_entropy_predictions(args, model, loaders[parsedargs.split])

    # global
    x, acc_global = accuracy_vs_entropy_percentile(data)

    # par classe
    x0, acc_c0 = accuracy_vs_entropy_per_predicted_class(data, class_id=0)
    x1, acc_c1 = accuracy_vs_entropy_per_predicted_class(data, class_id=1)


    save_path = build_entropy_plot_path(
    dataset=args.dataset,
    fold=parsedargs.fold_dataset,
    split=split,                  # ex: "target" / "test"
    method_name=method_name,       # ex: "PixelCAM"
    sfda_method=parsedargs.sfda_method,       # ex: "cdcl"
    )

    save_and_show_accuracy_plot(
        x,
        acc_global,
        acc_c0,
        acc_c1,
        save_path=save_path,
        title="Accuracy vs prediction certainty (entropy)"
    )
